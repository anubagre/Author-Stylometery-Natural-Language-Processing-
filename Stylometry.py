...
for auth in authors:
    tokens=nltk.word_tokenize(federalist_by_author[auth])
    a_tokens[auth] = ([token for token in tokens if any(c.isalpha() for c in token)])
    token_lengths = [len(token) for token in a_tokens[auth]]
    len_dist[auth] = nltk.FreqDist(token_lengths)
    len_dist[auth].plot(15, title=auth)
